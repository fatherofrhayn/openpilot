...
from .config import FORKS_DIR, LOGS_DIR, OPENPILOT_SYMLINK  # Relative import
...
def delete_fork_branch(fork, branch, dry_run=False):
    ...
    # Prevent deleting the currently active fork
    if not dry_run and os.path.islink(OPENPILOT_SYMLINK) and os.readlink(OPENPILOT_SYMLINK) == fork_path:
        print(f"Error: Cannot delete active fork {fork} [{branch}]. Please swap to another fork first.")
        return
